...
export const ImageAnnotator: React.FC<ImageAnnotatorProps> = ({ imageUrl, findings }) => {
  ...
  const [imageInfo, setImageInfo] = useState<RenderedImageInfo | null>(null);
    ...
    <div className="flex flex-col gap-4 w-full">
      <div className="flex items-center justify-between">
        ...
        <div className="flex bg-gray-800/50 p-1 rounded-lg border border-gray-700/50">
          <button onClick={() => setViewMode('box')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${viewMode === 'box' ? 'bg-gray-700 text-white shadow-sm' : 'text-gray-400 hover:text-gray-200'}`}><ScanLine className="w-3.5 h-3.5" /><span>Boxes</span></button>
          <button onClick={() => setViewMode('heatmap')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${viewMode === 'heatmap' ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'text-gray-400 hover:text-gray-200'}`}><Flame className="w-3.5 h-3.5" /><span>Heatmap</span></button>
          <button onClick={() => setViewMode('combined')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${viewMode === 'combined' ? 'bg-indigo-500/20 text-indigo-400 border border-indigo-500/30' : 'text-gray-400 hover:text-gray-200'}`}><Layers className="w-3.5 h-3.5" /><span>Mixed</span></button>
        </div>
      </div>
      ...
      <div ref={containerRef} className="w-full bg-black/40 rounded-xl border border-gray-800 p-4 flex justify-center items-center min-h-[400px] h-[60vh] relative overflow-hidden">
            ...
            <div className="absolute" style={{ top: imageInfo.top, left: imageInfo.left, width: imageInfo.width, height: imageInfo.height }}>
                 {(viewMode === 'heatmap' || viewMode === 'combined') && (
                    <div className="absolute inset-0 z-10 pointer-events-none">
                        ...
                        return (
                            <div key={`heat-${idx}`} className="absolute mix-blend-screen opacity-70 animate-pulse" style={{ top: `${ymin * 100}%`, left: `${xmin * 100}%`, height: `${(ymax - ymin) * 100}%`, width: `${(xmax - xmin) * 100}%`, background: gradient, filter: 'blur(12px)' }} />
                        );
                        })}
                    </div>
                 )}
                 {(viewMode === 'box' || viewMode === 'combined') && visualFindings.map((finding, idx) => {
                    ...
                    return (
                        <div key={`box-${idx}`} className={`absolute z-20 cursor-help transition-all duration-200 ${isHovered ? 'z-30' : ''}`} style={{ top: `${ymin * 100}%`, left: `${xmin * 100}%`, height: `${(ymax - ymin) * 100}%`, width: `${(xmax - xmin) * 100}%` }} onMouseEnter={() => setHoveredIndex(idx)} onMouseLeave={() => setHoveredIndex(null)}>
                            ...
                            {isHovered && (
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-48 bg-gray-900/95 backdrop-blur-md text-white rounded-lg p-3 shadow-2xl border border-gray-700 animate-in fade-in slide-in-from-bottom-2 duration-200 pointer-events-none">
                                    ...
                                    <div className="flex justify-between items-center text-[10px] text-gray-400 uppercase tracking-wider font-medium"><span>Confidence</span><span className="text-primary">{finding.confidence}</span></div>
        ...
        {!hasFindings && !imageInfo && (
            <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-md border border-white/10 text-white/80 text-xs px-3 py-1.5 rounded-full flex items-center gap-2">
                <Info className="w-3 h-3" /> Awaiting image analysis
            </div>
        )}
      </div>
